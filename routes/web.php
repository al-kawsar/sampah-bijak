...
Route::name('app.')->middleware('auth')->group(function () {
    ...
    // * DASHBOARD API
    Route::resource('events', EventController::class)->parameters(['events' => 'id:id']);
    ...
    Route::name('settings.')->controller(SettingController::class)->prefix('settings')->group(function () {
        ...
        Route::put('profile/{id:id}', 'updateProfile')->name('profile.update');
        Route::get('account', 'accountPage')->name('account');
        Route::get('account/{id:id}', 'updateAccount')->name('account.update');
    });
