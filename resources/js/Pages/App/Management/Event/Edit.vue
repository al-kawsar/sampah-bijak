<script setup>
  ...
  const handleSubmit = () => {
    ...
    formData.append("date", dayjs(form.value.date).format('YYYY-MM-DD HH:mm:ss'));    formData.append("location", form.value.location);
    formData.append("capacity", parseInt(form.value.capacity, 10));

    formData.forEach((value, key) => console.log(`${key}: ${value}`));


    axios.put(route("app.events.update", props.event.id), formData, {
      headers: {
  ...
  onMounted(() => {
    ...
    form.value = {
      ...
      date: dayjs(props.event.date, 'YYYY-MM-DD HH:mm:ss'),
      location: props.event.location,
      thumbnail: null,
      capacity: props.event.capacity,
    };

    if (props.event.thumbnail_url) {
      imageUrl.value = props.event.thumbnail_url;
    }
  });
</script>
...
<template>
  <div class="p-6">
    <div class="mb-6">
      <h1 class="text-2xl font-bold">Edit Acara</h1>
    </div>
